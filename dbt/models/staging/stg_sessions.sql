...
with events as (
  select user_id, ts, 'page_view'    as event_type from {{ ref('stg_page_view') }}
  union all
  select user_id, ts, 'add_to_cart'  as event_type from {{ ref('stg_add_to_cart') }}
  union all
  select user_id, ts, 'order_placed' as event_type from {{ ref('stg_order_placed') }}
),
ordered as (
  select
    user_id,
